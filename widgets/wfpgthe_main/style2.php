        ...
        while ( $woofpt_all_products->have_posts() ) : $woofpt_all_products->the_post();
            ...
            <div class="col-lg-4 col-md-4 col-sm-6 column_number woofpt_gap_c">
                <div class="woofpt-card">
					...
					<?php } else{echo '<div class="woofpt-img_text">'.esc_html__('No Feature Image', 'bwd-elementor-addons').'</div>';} ?>
                    <?php if('yes' === $settings['woofpt_cart_btn']){ ?>
                    <div class="woofpt-card-icon">
                        <?php echo '<a href="' . esc_url($product->add_to_cart_url()) . '"><i class="'.esc_attr($settings['woofpt_cart_button_icon']['value']).'"></i></a>'; ?>
                    </div>
                    <?php } ?>
                    <div class="woofpt-contentbx">
						<?php if('yes' === $settings['woofpt_title']){ ?>
                        <div class="woofpt-card-heading <?php echo esc_attr($woofpt_cont_align); ?>"><a href="<?php echo esc_url(get_permalink()); ?>"><?php echo esc_html(get_the_title()); ?></a></div>
						<?php } ?>
                       ...
                       <div class="woofpt-category-tag <?php echo esc_attr($woofpt_cont_align); ?>">
                        ...
                        <div class="woofpt-btn-ptype <?php echo esc_attr($woofpt_cont_align); ?>">
                            ...
                            <div class="woofpt-card-button">
                                <a href="<?php if('default' === $woofpt_choose_link){ echo esc_url(get_permalink()); } elseif('coustom_link' === $woofpt_choose_link){ echo esc_url($settings['woofpt_main_button_link']['url']); } ?>"><?php echo esc_html($woofpt_main_button, 'bwd-elementor-addons'); ?></a>
                            </div>
							<?php } ?>
